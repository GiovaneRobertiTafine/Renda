...
export class RecordService {
    ...
    get(): Observable<Record[]> {
        if (!this.loaded) {
            combineLatest(this.http.get<Record[]>(this.url), this.categorieService.get())
                .pipe(
    ...
    add(rec: Record): Observable<Record> {
        ...
        return this.http
            .post<Record>(this.url, { ...rec, categorie })
            .pipe(
                tap((r) => {
                    this.resourcesSubject$.getValue().push({ ...rec, _id: r._id });
    ...
    del(rec: Record): Observable<any> {
        return this.http.delete(`${this.url}/${rec._id}`).pipe(
            tap(() => {
    ...
    update(rec: Record): Observable<any> {
        let categorie = rec.categorie as Categorie;
        return this.http
            .patch<Record>(`${this.url}/${rec._id}`, { ...rec, categorie })
            .pipe(
                tap(() => {
                    let records = this.resourcesSubject$.getValue();
                    let i = records.findIndex((r) => r._id === rec._id);
