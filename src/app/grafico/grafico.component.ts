...
export class GraficoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.recordService.get()
      ...
      .subscribe((recs) => {
        ...
        (err) => err,
        () => {
          // console.log(this.searchMonth(this.records[0]))
          this.totalPerMonth();
        });
  ...
  totalPerMonth() {
    ...
    const typeCharge = {
      ...
      unic(res, mes) {
        res.type === 'renda' ?
          this.receita[mes] += res.value :
          this.despesa[mes] += res.value;
      },
      fix(res, mes) {
        if (res.type === 'renda') {
    ...
    console.log(this.totalDespesa)

    this.totalReceita.map((res, i) => {
      this.rendaMensal[i] = this.totalReceita[i] - this.totalDespesa[i];
      i != 0 ? this.rendaAcumulativa[i] = this.rendaMensal[i] + this.rendaAcumulativa[i - 1] :
        this.rendaAcumulativa[i] = this.rendaMensal[i];

    })

    console.log(this.rendaMensal)
    console.log(this.rendaAcumulativa)
